...
export const addNotification = async (id: string, userId: string, subject: string) => {
  ...
  notification.message = await setNotificationMessage(notification, notification.users.length, action, podcast!.title);
...
export const removeLikeNotification = async (id: string, userId: string) => {
  ...
  if (notification!.users.length === 1) {
    ...
    notification = await Notification.findByIdAndUpdate(notificationId, { $pull: { users: userId } }, { new: true });
